package pay

import (
	"context"
	"encoding/json"
	"fmt"
	"testing"

	"github.com/openweb3-io/pay-openapi/go/internal/openapi"
)

var (
	userId     = "testuser"
	appId      = "app_2jgyzxZEGTbJYky5vM8Xg4qs1yu"
	apikey     = "ak_faa1196ffd88494caee47f7808564b3e1a95"
	privateKey = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
)

func TestPayment_List(t *testing.T) {
	context := context.Background()
	client := New(apikey, privateKey, nil)
	response, err := client.Payment.List(context, appId, &PaymentListOptions{
		UserId: &userId,
	})
	if err != nil {
		t.Error(err)
	}
	buf, _ := json.MarshalIndent(response, "", "  ")
	fmt.Println(string(buf))
}

func TestPayment_Create(t *testing.T) {
	context := context.Background()
	client := New(apikey, privateKey, nil)
	response, err := client.Payment.Create(context, appId, &openapi.CreatePaymentRequest{
		Amount:   "1",
		Currency: "USDT_SOL",
		UserId:   &userId,
		Metadata: map[string]interface{}{
			"walletId": "test",
		},
		Extra: map[string]interface{}{
			"walletId": "test",
		},
		Provider: "openweb3_wallet",
	})
	if err != nil {
		t.Error(err)
	}
	// t.Log(fmt.Sprintf("%+v", response))
	buf, _ := json.MarshalIndent(response, "", "  ")
	fmt.Println(string(buf))
}

func TestEndpoint_List(t *testing.T) {
	context := context.Background()
	client := New(apikey, privateKey, nil)
	response, err := client.Endpoint.List(context, appId, &EndpointListOptions{
		Limit: PtrInt32(10),
	})
	if err != nil {
		t.Error(err)
	}
	buf, _ := json.MarshalIndent(response, "", "  ")
	fmt.Println(string(buf))
}

func TestEndpoint_Patch(t *testing.T) {
	context := context.Background()
	client := New(apikey, privateKey, nil)
	response, err := client.Endpoint.Patch(context, appId, "endpoint_2jgyzxZEGTbJYky5vM8Xg4qs1yu", &EndpointPatch{
		Url: PtrString("https://example.com"),
	})
	if err != nil {
		t.Error(err)
	}
	buf, _ := json.MarshalIndent(response, "", "  ")
	fmt.Println(string(buf))
}
